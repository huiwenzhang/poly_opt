...
inline std::ostream& operator<<(std::ostream& stream, const OptimizationInfo& val) {
    ...
    stream << "  stopping reason:       " << nlopt::returnValueToString(val.stopping_reason)
           << std::endl;
    ...
    for (const std::pair<int, Extremum>& m : val.maxima) {
        stream << "    " << positionDerivativeToString(m.first) << ": " << m.second.value
               << " in segment " << m.second.segment_idx << " and segment time " << m.second.time
               << std::endl;
    }
    return stream;
}
...
PolynomialOptimizationNonLinear<_N>::PolynomialOptimizationNonLinear(
        ...
        : poly_opt_(dimension), opt_params_(parameters) {}
...
bool PolynomialOptimizationNonLinear<_N>::setupFromVertices(
    ...
    size_t n_optimization_parameters;  // free variables in nlopt
    switch (opt_params_.time_alloc_method) {
    ...
    nlopt_.reset(new nlopt::opt(opt_params_.algorithm, n_optimization_parameters));
    nlopt_->set_ftol_rel(opt_params_.f_rel);
    nlopt_->set_ftol_abs(opt_params_.f_abs);
    nlopt_->set_xtol_rel(opt_params_.x_rel);
    nlopt_->set_xtol_abs(opt_params_.x_abs);
    nlopt_->set_maxeval(opt_params_.max_iters);

    if (opt_params_.random_seed < 0)
        nlopt_srand_time();
    else
        nlopt_srand(opt_params_.random_seed);
...
int PolynomialOptimizationNonLinear<_N>::optimizeTime() {
    ...
    for (double t : segment_times) { initial_step.push_back(opt_params_.initial_stepsize_rel * t); }
    ...
    try {
        ...
        // mwarn("error while setting up nlopt: %s", e.what());
    ...
    try {
        ...
        // mwarn("error while running nlopt: %s", e.what());
        return nlopt::FAILURE;
    }
    return result;
}
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeMellingerOuterLoop() {
    ...
    if (opt_params_.print_debug_info_time_allocation) {
        std::cout << "Segment times: ";
        for (const double seg_time : segment_times) { std::cout << seg_time << " "; }
        std::cout << std::endl;
    }
    ...
    try {
        ...
        // mwarn("error while running nlopt: %s", e.what());
        if (final_cost == std::numeric_limits<double>::max()) { return nlopt::FAILURE; }
    ...
    if (opt_params_.print_debug_info_time_allocation) {
        ...
        std::cout << "[MEL   Trajectory Time] Before: "
                  << std::accumulate(
                             original_segment_times.begin(), original_segment_times.end(), 0.0)
                  << " | After Rel Change: "
                  << std::accumulate(
                             relative_segment_times.begin(), relative_segment_times.end(), 0.0)
                  << " | After Scaling: "
...
double PolynomialOptimizationNonLinear<_N>::getTotalCostWithSoftConstraints() const {
    ...
    double cost_time = total_time * total_time * opt_params_.time_penalty;
    double cost_constraints = evaluateMaximumMagnitudeAsSoftConstraint(
            inequality_constraints_, opt_params_.soft_constraint_weight, 1e9);
...
double PolynomialOptimizationNonLinear<_N>::getCostAndGradientMellinger(
    ...
    if (gradients != NULL) {
        ...
        for (size_t n = 0; n < n_segments; n++) {
            ...
            for (double& t : segment_times_bigger) { t = std::max(kOptimizationTimeLowerBound, t); }
...
void PolynomialOptimizationNonLinear<_N>::scaleSegmentTimesWithViolation() {
    ...
    poly_opt_.updateSegmentTimes(segment_times);
    poly_opt_.solveLinear();  // why don't get coeffs from trajectory
...
int PolynomialOptimizationNonLinear<_N>::optimizeTimeAndFreeConstraints() {
    ...
    poly_opt_.getFreeConstraints(&free_constraints);  // continuous constraints
    if (free_constraints.size() == 0 || free_constraints.front().size() == 0) {
        // mwarn("No free derivative variables, same as time-only optimization.");
    }

    // 自由变量，flatten to a vector
    const size_t n_optimization_variables =
            n_segments + free_constraints.size() * free_constraints.front().size();

    assert(n_optimization_variables > 0u);
    ...
    for (double t : segment_times) { initial_solution.push_back(t); }

    for (const Eigen::VectorXd& c : free_constraints) {
        for (int i = 0; i < c.size(); ++i) { initial_solution.push_back(c[i]); }
    }
    ...
    lower_bounds.insert(
            std::end(lower_bounds), std::begin(lower_bounds_free), std::end(lower_bounds_free));
    upper_bounds.insert(
            std::end(upper_bounds), std::begin(upper_bounds_free), std::end(upper_bounds_free));
    ...
    for (size_t i = 0; i < initial_solution.size(); i++) {
        ...
        if (abs_x <= std::numeric_limits<double>::lowest()) {
            ...
            initial_step.push_back(opt_params_.initial_stepsize_rel * abs_x);
        }
    ...
    assert(initial_solution.size() == lower_bounds.size());
    assert(initial_solution.size() == initial_step.size());
    assert(initial_solution.size() == n_optimization_variables);
    ...
    try {
        ...
        nlopt_->set_min_objective(
                &PolynomialOptimizationNonLinear<N>::objectiveFunctionTimeAndConstraints, this);
    } catch (std::exception& e) {
        // mwarn("error while setting up nlopt: %s", e.what());
        return nlopt::FAILURE;
    }
...
bool PolynomialOptimizationNonLinear<_N>::addMaximumMagnitudeConstraint(int derivative,
    ...
    if (!opt_params_.use_soft_constraints) {
        try {
            nlopt_->add_inequality_constraint(
                    &PolynomialOptimizationNonLinear<N>::evaluateMaximumMagnitudeConstraint,
                    constraint_data.get(),
                    opt_params_.inequality_constraint_tolerance);
        } catch (std::exception& e) {
            // mwarn("ERROR while setting inequality constraint %s", e.what());
            return false;
        }
    }
    return true;
...
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTime(
        ...
        void* data) {
    assert(gradient.empty());  // only gradient-free methods valid
    assert(data != nullptr);

    PolynomialOptimizationNonLinear<N>* optimization_data =
            static_cast<PolynomialOptimizationNonLinear<N>*>(data);  // wheee ...

    assert(segment_times.size() == optimization_data->poly_opt_.getNumberSegments());
    ...
    switch (optimization_data->opt_params_.time_alloc_method) {
    case NonlinearOptimizationParameters::kRichterTime:
        cost_time = total_time * optimization_data->opt_params_.time_penalty;
        break;
    default:  // kSquaredTime
        cost_time = total_time * total_time * optimization_data->opt_params_.time_penalty;
        break;
    }

    if (optimization_data->opt_params_.print_debug_info) {
        std::cout << "---- cost at iteration " << optimization_data->opt_info_.n_iterations
                  << "---- " << std::endl;
        std::cout << "  trajectory: " << cost_trajectory << std::endl;
        std::cout << "  time: " << cost_time << std::endl;
    }

    if (optimization_data->opt_params_.use_soft_constraints) {
        cost_constraints = optimization_data->evaluateMaximumMagnitudeAsSoftConstraint(
                optimization_data->inequality_constraints_,
                optimization_data->opt_params_.soft_constraint_weight);
    }
    ...
    optimization_data->opt_info_.n_iterations++;
    optimization_data->opt_info_.cost_trajectory = cost_trajectory;
    optimization_data->opt_info_.cost_time = cost_time;
    optimization_data->opt_info_.cost_soft_constraints = cost_constraints;
...
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTimeMellingerOuterLoop(
    ...
    // why we can access to the private data poly_opt
    assert(segment_times.size() == optimization_data->poly_opt_.getNumberSegments());
    ...
    if (optimization_data->opt_params_.print_debug_info) {
        std::cout << "---- cost at iteration " << optimization_data->opt_info_.n_iterations
                  << "---- " << std::endl;
        std::cout << "  segment times: ";
        for (double segment_time : segment_times) { std::cout << segment_time << " "; }
        std::cout << std::endl;
...
double PolynomialOptimizationNonLinear<_N>::objectiveFunctionTimeAndConstraints(
    ...
    assert(x.size() == n_segments + n_free_constraints * dim);
    ...
    for (size_t d = 0; d < dim; ++d) {
        // [t0, t1,..., free_cons_1[0], free_cons_2[0], ..., free_cons_1[1]...]
...
double PolynomialOptimizationNonLinear<_N>::evaluateMaximumMagnitudeConstraint(
    ...
    Extremum max;
    max = optimization_data->poly_opt_.computeMaximumOfMagnitude(constraint_data->derivative,
                                                                 nullptr);

    optimization_data->opt_info_.maxima[constraint_data->derivative] = max;
...
double PolynomialOptimizationNonLinear<_N>::evaluateMaximumMagnitudeAsSoftConstraint(
    ...
    if (opt_params_.print_debug_info) std::cout << "  soft_constraints: " << std::endl;
    ...
    for (std::shared_ptr<const ConstraintData> constraint : inequality_constraints_) {
        ...
        // void*.
        double abs_violation =
                evaluateMaximumMagnitudeConstraint(dummy, dummy, (void*) constraint.get());
        ...
        cost += current_cost;
        if (opt_params_.print_debug_info) {
