...
public:
    ...
    void addSegments(const Segment::Vector& segments) {
        for (const Segment& segment : segments) {
            assert(segment.D() == D_);
            assert(segment.N() == N_);
            max_time_ += segment.getTime();  // 每段segment的时间
        }
    ...
    bool getVertices(int max_derivative_order_pos,
    ...
    Eigen::VectorXd evaluate(double t,
                             int derivative_order = derivative_order::POSITION) const;
    ...
    void evaluateRange(double t_start,
                       double t_end,
                       double dt,
    ...
    void evaluateRange(double t_start,
                       ...
                       int derivative_order,
                       std::vector<Eigen::VectorXd>* result,
                       std::vector<size_t>* size_of_segment) const;
    ...
    // 用解析求根法得到每段多项式极值，然后得到全局极值
    bool computeMaxVelocityAndAcceleration(double* v_max, double* a_max) const;

    // 利用离散化的方法得到极值
    double getMaximumMagnitude(size_t derivative, double dt = 0.01);
