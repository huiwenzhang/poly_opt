...
bool Segment::operator==(const Segment& rhs) const {
    if (D_ != rhs.D_ || time_ != rhs.time_) {
        ...
        for (int i = 0; i < D(); i++) {
            if (polynomials_[i] != rhs[i]) { return false; }
        }
    }
    return true;
}
...
Polynomial& Segment::operator[](size_t idx) {
    assert(idx < static_cast<size_t>(D_));
...
Eigen::VectorXd Segment::evaluate(double t, int derivative) const {
    ...
    result.setZero();
    for (int d = 0; d < D_; ++d) { result[d] = polynomials_[d].evaluate(t, derivative); }
    return result;
}

void printSegment(std::ostream& stream, const Segment& s, int derivative) {
    assert(derivative >= 0 && derivative < s.N());
...
bool Segment::computeMinMaxMagnitudeCandidateTimes(
    ...
    assert(candidate_times != nullptr);
    candidate_times->clear();
    // Compute magnitude derivative roots.
    if (dimensions.empty()) {
        // mwarn("No dimensions specified.");
        return false;
        ...
        for (int dim : dimensions) {
            if (dim < 0 || dim >= D_) {
                // mwarn("Specified dimension are out of bound [0 %d]", D_);
                return false;
            }
        ...
        // integral.
        if (!polynomial_convolved.computeMinMaxCandidates(
                    t_start, t_end, -1, candidate_times)) {
            return false;
        }
    } else {
...
bool Segment::computeMinMaxMagnitudeCandidates(int derivative,
                                               double t_start,
                                               double t_end,
                                               const std::vector<int>& dimensions,
    ...
    computeMinMaxMagnitudeCandidateTimes(
            derivative, t_start, t_end, dimensions, &candidate_times);
...
bool Segment::selectMinMaxMagnitudeFromCandidates(int derivative,
                                                  double t_start,
                                                  double t_end,
                                                  const std::vector<int>& dimensions,
    ...
    for (const Extremum& candidate : candidates) {
        if (candidate.time < t_start || candidate.time > t_end) { continue; }
...
bool Segment::getSegmentWithSingleDimension(int dimension, Segment* new_segment) const {
    if (dimension < 0 || dimension >= D_) {
        // mwarn("You shan't ask for a dimension that does not exist in the segment.");
        return false;
    }
...
bool Segment::getSegmentWithAppendedDimension(const Segment& segment_to_append,
    ...
    if (time_ < new_time && new_time > 0.0) {
        ...
        for (int d = 0; d < segment_to_append.D(); d++) {
            segment_to_append_temp[d].scalePolynomialInTime(segment_to_append.getTime() /
                                                            new_time);
        }
    }
...
bool Segment::offsetSegment(const Eigen::VectorXd& A_r_B) {
    if (A_r_B.size() < std::min(D_, 3)) {
        // mwarn("Offset vector size smaller than segment dimension.");
        return false;
    }
