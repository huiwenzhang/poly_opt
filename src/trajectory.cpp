...
bool Trajectory::operator==(const Trajectory& rhs) const {
    if (segments_.size() != rhs.segments_.size()) {
        ...
        for (int i = 0; i < K(); i++) {
            if (segments_ != rhs.segments_) { return false; }
        }
        return true;
    }
}
...
Eigen::VectorXd Trajectory::evaluate(double t, int derivative_order) const {
    ...
    for (i = 0; i < segments_.size(); ++i) {
        ...
        if (accumulated_time > t) { break; }
    }
    if (t > accumulated_time) {
        // mwarn("Time out of range of the trajectory!");
    ...
    if (i >= segments_.size()) { i = segments_.size() - 1; }
...
void Trajectory::evaluateRange(double t_start,
                               double t_end,
                               double dt,
    ...
    for (i = 0; i < segments_.size(); ++i) {
        ...
        // in case t_start falls on a vertex, the iterator right of the vertex is
        // chosen, hence accumulated_segment_time_ns > t_start
        if (accumulated_time > t_start) { break; }
    }
    if (t_start > accumulated_time) {
        // mwarn("Start time out of range of the trajectory!");
        return;
    }
    ...
    while (accumulated_time < t_end) {
        if (time_in_segment > segments_[i].getTime()) {
            ...
            if (i >= segments_.size()) { break; }
            continue;
        }

        result->push_back(segments_[i].evaluate(time_in_segment, derivative_order));

        if (sampling_times != nullptr) { sampling_times->push_back(accumulated_time); }
...
void Trajectory::evaluateRange(double t_start,
                               ...
                               int derivative_order,
                               std::vector<Eigen::VectorXd>* result,
                               std::vector<size_t>* size_of_each_segment) const {
    const size_t expected_number_of_samples = (t_end - t_start) / dt + 1;
    ...
    while (accumulated_time < t_end) {
        if (time_in_segment > segments_[i].getTime()) {
            time_in_segment = time_in_segment - segments_[i].getTime();
            size_of_each_segment->push_back(cnt);
    ...
    size_of_each_segment->push_back(cnt);  // the last segment size
}
...
bool Trajectory::getTrajectoryWithAppendedDimension(const Trajectory& trajectory_to_append,
    ...
    assert(static_cast<int>(segments_.size()) == trajectory_to_append.K());
    ...
    for (size_t k = 0; k < segments_.size(); ++k) {
        ...
        if (!segments_[k].getSegmentWithAppendedDimension(trajectory_to_append.segments()[k],
                                                          &new_segment)) {
            return false;
        }
...
std::vector<double> Trajectory::getSegmentTimes() const {
    ...
    for (size_t i = 0; i < segment_times.size(); ++i) { segment_times[i] = segments_[i].getTime(); }
...
bool Trajectory::addTrajectories(const std::vector<Trajectory>& trajectories,
    ...
    *merged = *this;
    ...
    for (const Trajectory& t : trajectories) {
        ...
        if (t.D() != D_ || t.N() != N_) {
            // mwarn("Dimension to append [%d -> %d] or number of coeffs [%d -> %d] doesn't "
            //       "match",
            //       t.D(),
            //       D_,
            //       t.N(),
...
bool Trajectory::offsetTrajectory(const Eigen::VectorXd& A_r_B) {
    if (A_r_B.size() < std::min(D_, 3)) {
        // mwarn("Offset vector size smaller than trajectory dimension.");
        return false;
    }
...
Vertex Trajectory::getVertexAtTime(double t, int max_derivative_order) const {
    Vertex v(D_);
    for (size_t i = 0; i <= max_derivative_order; i++) { v.addConstraint(i, evaluate(t, i)); }
    return v;
}
...
bool Trajectory::getVertices(int max_derivative_order_pos,
    ...
    assert(pos_vertices != nullptr && yaw_vertices != nullptr);
    ...
    if (!temp_vertex.getSubdimension(
                kPosDimensions, max_derivative_order_pos, &pos_vertices->front()))
        return false;
    if (!temp_vertex.getSubdimension(
                kYawDimensions, max_derivative_order_yaw, &yaw_vertices->front()))
        return false;
    ...
    for (size_t i = 0; i < segments_.size(); ++i) {
        ...
        if (!temp_vertex.getSubdimension(
                    kPosDimensions, max_derivative_order_pos, &(*pos_vertices)[i + 1]))
            return false;
        if (!temp_vertex.getSubdimension(
                    kYawDimensions, max_derivative_order_yaw, &(*yaw_vertices)[i + 1]))
            return false;
    }
    return true;
}
...
bool Trajectory::computeMaxVelocityAndAcceleration(double* v_max, double* a_max) const {
    ...
    std::iota(dimensions.begin(), dimensions.end(), 0);  // increase sequence step =1

    Extremum v_min_traj, v_max_traj, a_min_traj, a_max_traj;

    bool success = computeMinMaxMagnitude(
            poly_opt::derivative_order::VELOCITY, dimensions, &v_min_traj, &v_max_traj);
    success &= computeMinMaxMagnitude(
            poly_opt::derivative_order::ACCELERATION, dimensions, &a_min_traj, &a_max_traj);
...
double Trajectory::getMaximumMagnitude(size_t derivative, double dt) {
    double maximum = std::numeric_limits<double>::lowest();
    for (double t = 0; t < max_time_; t += dt) {
        double current_value = evaluate(t, derivative).norm();
...
bool Trajectory::scaleSegmentTimesToMeetConstraints(double v_max, double a_max) {
    ...
    for (size_t i = 0; i < kMaxCounter; i++) {
        ...
        double violation_scaling =
                std::max(1.0, std::max(velocity_violation, sqrt(acceleration_violation)));
